...
class StructuralLF(BaseLF):
    ...
    def find_threshold(self, labeled_dataset: BaseDataset, unlabeled_dataset: BaseDataset, beta=0.1) -> None:
        ...
        X_lab = labeled_dataset.features_2[:, self.features_combs]
        X_unlab = unlabeled_dataset.features_2[:, self.features_combs]
    ...
    def get_labels(self, dataset: BaseDataset) -> List:
        probs = self.lf.predict_proba(dataset.features_2[:, self.features_combs])
