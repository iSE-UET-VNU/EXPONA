...
class ProbLF(BaseLF):
    ...
    def save(self) -> Path:
        save_path = self.lf_path / f"lf_{self.find_available_index()}.pkl"
        payload = {
            "model": self.lf,
    ...
    def load_lf(self, path: Path):
        ...
        self.lf = payload["model"]
        self.threshold = payload["threshold"]
        self.estimated_fbeta = payload["estimated_fbeta"]
        self.estimated_performance = payload["estimated_performance"]
        self.estimated_coverage = payload["estimated_coverage"]

    def _find_threshold(self, labeled_features, unlabeled_features, labels, beta=0.1, using_unlabeled=True):
