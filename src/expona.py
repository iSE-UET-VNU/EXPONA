...
from .label_function import SurfaceLF, StructuralLF, SemanticLF
from .utils import (
    get_labeled_subset, 
    get_wrench_model, 
    generate_semantic_lfs, 
    generate_structural_lfs, 
    generate_surface_lfs, 
...
class Expona:
    def __init__(self, args):
        ...
        display_label_distribution("Train", self.train_data)
        display_label_distribution("Valid", self.valid_data)
        display_label_distribution("Test", self.test_data)
    ...
    def filter_lf(self, lf, acceptable_performance, acceptable_coverage):
        ...
        for i, h in enumerate(lf):
            if h.estimated_performance < acceptable_performance:
                logger.debug(f"Skipping lf with estimated_performance {h.estimated_performance:.4f} below min acceptable_performance {acceptable_performance:.4f}")
            elif h.estimated_coverage < acceptable_coverage:
                logger.debug(f"Skipping lf with estimated_coverage {h.estimated_coverage:.4f} below min acceptable_coverage {acceptable_coverage:.4f}")
            else:
    ...
    def evaluate_lfs(self):
        ...
        for label_model_name in self.LM_NAMES:
            ...
            self.aggregated_probs = train_prob_label_covered
            train_label_covered = best_label_model.predict(train_data_covered)  
            self.aggregated_labels = train_label_covered
            ...
            logger.info(f"[{label_model_name}] Covered Train — Acc: {lm_acc_train:.4f}, F1: {lm_f1_train:.4f}, Coverage: {coverage:.4f}")
            ...
            for end_model_name in self.EM_NAMES:
                ...
                for T2 in range(self.args.eval_times):
                    ...
                    logger.debug(f"  [{T2+1}/{self.args.eval_times}] {label_model_name}+{end_model_name} — Acc: {em_acc:.4f}, F1: {em_f1:.4f}")
                ...
                logger.info(f"[{label_model_name}+{end_model_name}] Test — Acc: {em_acc_mean:.4f}±{em_acc_std:.4f}, F1: {em_f1_mean:.4f}±{em_f1_std:.4f}")
    ...
    def save_results_json(self, output_dir="output"):
        ...
        output_file = output_dir / f"{self.args.exp_name}_{self.args.cur_time}.json"
        
        data_to_save = {
            "dataset": self.args.dataset,
            "version": self.args.exp_name,
    ...
    def display_stats(self):
        for split, data in zip(["train", "valid", "test"], [self.train_data, self.valid_data, self.test_data]):
            logger.debug(f"{split.capitalize()} Data LF summary:")
            logger.debug(data.lf_summary())
    ...
    def run_exp(self):
        ...
        while self.check_termination():
            ...
            if self.args.min_lf_per_type - len(self.cur_surface_lf) > 0:
                n = self.args.min_lf_per_type - len(self.cur_surface_lf)
                logger.info(f"Generating {n} surface LFs...")
                self.cur_surface_lf.extend(self.generate_lf(LFType.SURFACE, n))
            if self.args.min_lf_per_type - len(self.cur_structural_lf) > 0:
                n = self.args.min_lf_per_type - len(self.cur_structural_lf)
                logger.info(f"Generating {n} structural LFs...")
                self.cur_structural_lf.extend(self.generate_lf(LFType.STRUCTURAL, n))
            if self.args.min_lf_per_type - len(self.cur_semantic_lf) > 0:
                n = self.args.min_lf_per_type - len(self.cur_semantic_lf)
                logger.info(f"Generating {n} semantic LFs...")
                self.cur_semantic_lf.extend(self.generate_lf(LFType.SEMANTIC, n))
        ...
        start = time.time()
        logger.info("\n\nDone generating and filtering LFs. Evaluating LFs...")
        self.evaluate_lfs()
        ...
        all_lfs = self.cur_surface_lf + self.cur_structural_lf + self.cur_semantic_lf
        for lf in all_lfs:
            if not lf.is_saved:
                lf.save()
