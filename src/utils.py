...
def display_label_distribution(name, dataset):
    labels = np.array(dataset.labels)
    counter = Counter(labels)
    total = len(labels)
    logger.debug(f"{name} Label Distribution:")
    for label, count in sorted(counter.items()):
        logger.debug(f"  Label {label}: {count} ({count/total:.2%})")
...
def display_lf_stats(heuristic):
    logger.debug("="*50)
    logger.debug(f"Type of LF: {heuristic.__class__}")
    logger.debug(f"Estimated Performance: {heuristic.estimated_performance:.4f}")
    logger.debug(f"Estimated Coverage: {heuristic.estimated_coverage:.4f}")
    logger.debug(f"Estimated WHM: {heuristic.estimated_fbeta:.4f}")
    logger.debug("="*50)
...
def generate_surface_lfs(
    ...
    for response in response_list:
        ...
        if SurfaceLF.is_runnable_lf(code_str=code_str, test_input=labeled_dataset.examples[0]["text"]):
            ...
            lf.estimate_performance(
                labeled_dataset=labeled_dataset, 
                unlabeled_dataset=unlabeled_dataset,
                beta=args.beta,
                using_unlabeled=args.using_unlabeled
            )
            lf_list.append(lf)
...
def generate_structural_lfs(
    ...
    for _ in range(num_trials):
        ...
        generator = ScikitGenerator(train_labeled.features_struc, train_labeled.labels)
        lf = generator.generate_lfs(model=args.structural_lf_model)
...
def generate_semantic_lfs(
    ...
    for _ in range(num_trials):
        ...
        lf = generator.generate_lfs(model=args.semantic_lf_model)
