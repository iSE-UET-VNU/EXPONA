...
class BaseDataset(ABC):
    ...
    def load_features(self, cache_name: Optional[str] = None, is_struc: bool = False):
        ...
        if is_struc:
            path = self.path / f'{self.split}_{cache_name}_struc.pkl'
        else:
        ...
        if is_struc:
            self.features_struc = features
        else:
            self.features = features
        return features

    def save_features(self, cache_name: Optional[str] = None, is_struc: bool = False):
        if cache_name is None:
            return None
        if is_struc:
            path = self.path / f'{self.split}_{cache_name}_struc.pkl'
            logger.info(f'saving features into {path}')
            pickle.dump(self.features_struc, open(path, 'wb'), protocol=4)
        else:
            path = self.path / f'{self.split}_{cache_name}.pkl'
            logger.info(f'saving features into {path}')
    ...
    def extract_feature(self,
        ...
        if cache_name is not None:
            ...
            if path.exists() and (not force):
                self.load_features(cache_name=cache_name, is_struc=is_struc)
                return

        if isinstance(extract_fn, Callable):
            if is_struc:
                self.features_struc = extract_fn(self.examples)
            else:
                self.features = extract_fn(self.examples)
        else:
            extractor = self.extract_feature_(extract_fn=extract_fn, return_extractor=return_extractor, is_struc=is_struc, **kwargs)
            if normalize:
        ...
        if cache_name is not None:
            self.save_features(cache_name=cache_name, is_struc=is_struc)

        if return_extractor:
            return extract_fn

    @abstractmethod
    def extract_feature_(self, extract_fn: str, return_extractor: bool, is_struc: bool = False, **kwargs: Any):
    ...
    def create_subset(self, idx: List[int]):
        ...
        if self.features_struc is not None:
            dataset.features_struc = self.features_struc[idx]
