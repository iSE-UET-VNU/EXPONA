...
text_datasets = ['agnews', 'imdb', 'sms', 'trec', 'yelp', 'youtube', 'medabs', 'clickbait', 'finance', 'tos', 'chemprot', 'massive', 'paper', 'pubmed', 'movie_genre', 'vnu']
...
class StructuredEmbeddingFactory:
...
def load_dataset(data_home, dataset, dataset_type=None, label_number=None, extract_feature=False, extract_fn=None, extract_fn_struc=None, **kwargs):
    ...
    if extract_feature and (dataset_class != BaseSeqDataset):        
        extract_fn_struc = train_data.extract_feature(extract_fn=extract_fn_struc, 
                                                                                 return_extractor=True, is_struc=True, **kwargs)
        valid_data.extract_feature(extract_fn=extract_fn_struc, 
                                                               return_extractor=False, 
                                                               is_struc=True, **kwargs)
        test_data.extract_feature(extract_fn=extract_fn_struc, 
                                                             return_extractor=False, 
                                                             is_struc=True, **kwargs)
        
        train_data.extract_feature(extract_fn=extract_fn, return_extractor=True, **kwargs)
        valid_data.extract_feature(extract_fn=extract_fn, return_extractor=False, **kwargs)
        test_data.extract_feature(extract_fn=extract_fn, return_extractor=False, **kwargs)

    return train_data, valid_data, test_data


def load_image_dataset(data_home, dataset, image_root_path, preload_image=True, extract_feature=False, extract_fn='pretrain', extract_fn_struc=None, **kwargs):
    dataset_path = Path(data_home) / dataset
    ...
    if extract_feature:
        ...
        train_data.extract_feature(extract_fn=extract_fn_struc, return_extractor=True, is_struc=True, **kwargs)
        valid_data.extract_feature(extract_fn=extract_fn_struc, return_extractor=False, is_struc=True, **kwargs)
        test_data.extract_feature(extract_fn=extract_fn_struc, return_extractor=False, is_struc=True, **kwargs)
    return train_data, valid_data, test_data